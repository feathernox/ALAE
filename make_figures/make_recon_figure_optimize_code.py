...
def sample(cfg, logger):
    ...
    def optimize(x, latent, mse_coef=10, initial_lr=0.1, step=1000, lr_rampdown=0.25, lr_rampup=0.05):
        ...
        for i in pbar:
            ...
            mse_loss = F.mse_loss(output_resized, x_resized, reduction='none'
    ...
    paths = chunker_list(paths[:6], 8 * 3)

    for i, chunk in enumerate(paths):
        canvas = make(chunk)
        canvas = torch.cat(canvas, dim=0)

        save_path = 'make_figures/output/%s/reconstructions_%d.png' % (cfg.NAME + '_opt2', i)
